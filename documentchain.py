...
def main():
    try:
        ...
        for i in statelist: 
            ...
            for i in userslist:  
                ...
                for txnlist in txncounter:
                    ...
                    # Get Document Type
                    doc_type_name = functions.getDocumentType(txnlist["state_mnemonic"], txnlist["department_mnemonic"], txnlist["certtype_mnemonic"])
                ...
                nic_body += body
                emailid = i["email"]
                subject="Document Chain Statistics for "+yesterday_date_str_1
                fname = "emailtemplate_nic.htm"
                html_file = open(fname, 'r', encoding='utf-8')
                source_code = html_file.read() 
                source_code = source_code.replace("@chainname", 'Document Chain')
        ...
        file_name = 'document_chain_mail_'+yesterday_date_str_1+'.json'
        if dir_path.is_dir():
        ...
        delete_file_name='document_chain_mail_'+delete_file_date_str+'.json'
